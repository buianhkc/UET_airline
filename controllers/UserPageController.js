...
class UserPageController {
    ...
    payTicket(req, res) {
        ...
        var sql = `SELECT ma_ve, trang_thai, so_luong_nguoi_lon + so_luong_tre_em as totalHuman , so_luong_nguoi_lon * gia_ve_nguoi_lon + so_luong_tre_em * gia_ve_tre_em as totalAmount
                    FROM orders
                    INNER JOIN tickets USING(ma_ve)
        ...
        db.execute(sql)
            ...
            .then((data) => {
                var { ma_ve, totalHuman, trang_thai, totalAmount } = data;
    ...
    deleteTicket(req, res) {
        ...
        db.execute(sql)
            ...
            .then((data) => {
                var sql = `UPDATE seat_details as sd
                    INNER JOIN(SELECT * 
                    FROM order_seats
                    WHERE order_number = ${order_number}) as tmp ON tmp.ma_chuyen_bay = sd.ma_chuyen_bay AND tmp.ma_ghe = sd.ma_ghe
            ...
            .then(data => {
                ...
                var q4 = `UPDATE tickets
                        SET so_luong_con = so_luong_con + ${totalHuman}
                        WHERE ma_ve = ${ma_ve}`
